...
const MahasiswaDashboardContent = ({ user, navigate }) => {
  ...
  //     try {
  //       // Ganti dengan API call Anda: const response = await axios.get(`${API_BASE_URL}/dashboard/mahasiswa/stats/${user.id}`);
  //       // setStats(response.data);
  //       setStats({ proyekDiajukan: 5, proyekAktif: 2, danaTerkumpulTotal: "Rp 12.5 Jt" }); // Placeholder
  //     } catch (error) { console.error("Gagal mengambil statistik mahasiswa:", error); }
  ...
  return (
    <div className="mt-8 space-y-8 animate-fadeInUp">
      <div className="grid md:grid-cols-3 gap-6">
        <StatCard title="Proyek Diajukan" value={stats.proyekDiajukan} icon={<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-8 h-8"><path strokeLinecap="round" strokeLinejoin="round" d="M9 12h3.75M9 15h3.75M9 18h3.75m3 .75H18a2.25 2.25 0 0 0 2.25-2.25V6.108c0-1.135-.845-2.098-1.976-2.134A48.664 48.664 0 0 0 12 3c-2.392 0-4.744.175-7.024.524C3.845 3.96 3 4.923 3 6.108V18.75c0 1.243.801 2.25 1.758 2.25H6.75m9.75-1.5h-3.75M15.75 9H18M15.75 12H18m-3.75 3H18M3.75 21v-6.375c0-.621.504-1.125 1.125-1.125H19.875c.621 0 1.125.504 1.125 1.125V21m-4.875-6.75h.008v.008h-.008v-.008Z" /></svg>} color="text-[#1dc071]" iconBgColor="bg-green-800/30"/>
        <StatCard title="Proyek Aktif" value={stats.proyekAktif} icon={<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-8 h-8"><path strokeLinecap="round" strokeLinejoin="round" d="M2.25 18 9 11.25l4.306 4.306a11.95 11.95 0 0 1 5.814-5.518l2.74-1.22m0 0-5.94-2.281m5.94 2.28-2.28 5.941" /></svg>} color="text-blue-400" iconBgColor="bg-blue-800/30"/>
        <StatCard title="Total Dana Terkumpul" value={stats.danaTerkumpulTotal} icon={<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-8 h-8"><path strokeLinecap="round" strokeLinejoin="round" d="M2.25 18.75a60.07 60.07 0 0 1 15.797 2.101c.727.198 1.453-.342 1.453-1.096V18.75M3.75 4.5v.75A.75.75 0 0 1 3 6A2.25 2.25 0 0 0 .75 8.25v.75A2.25 2.25 0 0 0 3 11.25v.75a2.25 2.25 0 0 0 2.25 2.25h15A2.25 2.25 0 0 0 22.5 12v-.75a2.25 2.25 0 0 0-2.25-2.25h-1.106a2.251 2.251 0 0 0-2.146-.991l-.691.097a2.25 2.25 0 0 1-2.032-.756l-.016-.024a2.25 2.25 0 0 0-2.412-.71l-.203.048a1.125 1.125 0 0 1-1.03-1.579l.168-.603A2.25 2.25 0 0 0 9.75 3H3.75Z" /></svg>} color="text-purple-400" iconBgColor="bg-purple-800/30"/>
      </div>
      <div className="bg-[#1c1c24] p-6 rounded-xl shadow-lg">
        ...
        <div className="flex flex-col sm:flex-row gap-4">
          <Link to="/create-campaign" className="flex-1"><CustomButton title="Ajukan Proyek Baru" styles="w-full bg-[#1dc071] hover:bg-green-600" /></Link>
          <Link to="/my-projects" className="flex-1"><CustomButton title="Kelola Proyek Saya" styles="w-full bg-blue-500 hover:bg-blue-600" /></Link>
          <Link to="/history" className="flex-1"><CustomButton title="Riwayat Transaksi" styles="w-full bg-gray-600 hover:bg-gray-700" /></Link>
        </div>
      </div>
      {/* Anda bisa menambahkan section notifikasi penting di sini jika ada */}
    </div>
  );
};

const DonaturDashboardContent = ({ user, navigate }) => {
  const [stats, setStats] = useState({ totalDonasi: "Rp 0", proyekDidukung: 0, nftDiterima: 0 });
  // ... logika fetch data dashboard donatur ...
  return (
    <div className="mt-8 space-y-8 animate-fadeInUp">
      <div className="grid md:grid-cols-3 gap-6">
        <StatCard title="Total Donasi Anda" value={stats.totalDonasi} icon={<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-8 h-8"><path strokeLinecap="round" strokeLinejoin="round" d="M21 11.25v8.25a1.5 1.5 0 0 1-1.5 1.5H5.25a1.5 1.5 0 0 1-1.5-1.5v-8.25M12 4.875A2.625 2.625 0 1 0 9.375 7.5H12m0-2.625V7.5m0-2.625A2.625 2.625 0 1 1 14.625 7.5H12m0 0H9.375m3.375 0A2.625 2.625 0 1 0 14.625 7.5M12 10.875v-3.375m0 0h3.375M12 10.875H9.375m1.5-3.375A2.625 2.625 0 1 1 12 4.875v0Z" /></svg>} color="text-[#8c6dfd]" iconBgColor="bg-purple-800/30"/>
        <StatCard title="Proyek Didukung" value={stats.proyekDidukung} icon={<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-8 h-8"><path strokeLinecap="round" strokeLinejoin="round" d="M9 12.75 11.25 15 15 9.75M21 12c0 1.268-.63 2.39-1.593 3.068a3.745 3.745 0 0 1-1.043 3.296A3.745 3.745 0 0 1 16.5 21a3.745 3.745 0 0 1-2.86-1.336A3.746 3.746 0 0 1 12 21a3.746 3.746 0 0 1-1.64-.336A3.745 3.745 0 0 1 7.5 21a3.746 3.746 0 0 1-2.86-1.336A3.745 3.745 0 0 1 3 19.664V18a3 3 0 0 1 3-3h2.25m6.75 0h2.25a3 3 0 0 1 3 3v1.664a3.745 3.745 0 0 1-1.64-.336A3.745 3.745 0 0 1 16.5 21A3.745 3.745 0 0 1 13.64 19.664Zm-9.75 0h2.25" /></svg>} color="text-green-400" iconBgColor="bg-green-800/30"/>
        <StatCard title="NFT Reward Diterima" value={stats.nftDiterima} icon={<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-8 h-8"><path strokeLinecap="round" strokeLinejoin="round" d="M2.25 7.125C2.25 6.504 2.754 6 3.375 6h6c.621 0 1.125.504 1.125 1.125v3.75c0 .621-.504 1.125-1.125 1.125h-6A1.125 1.125 0 0 1 2.25 10.875v-3.75ZM14.25 8.625c0-.621.504-1.125 1.125-1.125h5.25c.621 0 1.125.504 1.125 1.125v8.25c0 .621-.504 1.125-1.125 1.125h-5.25a1.125 1.125 0 0 1-1.125-1.125v-8.25ZM3.75 16.125c0-.621.504-1.125 1.125-1.125h5.25c.621 0 1.125.504 1.125 1.125v2.25c0 .621-.504 1.125-1.125 1.125h-5.25a1.125 1.125 0 0 1-1.125-1.125v-2.25Z" /></svg>} color="text-teal-400" iconBgColor="bg-teal-800/30"/>
      </div>
      <div className="bg-[#1c1c24] p-6 rounded-xl shadow-lg">
        <h3 className="font-epilogue font-semibold text-xl text-white mb-4">Aksi Cepat</h3>
        <div className="flex flex-col sm:flex-row gap-4">
            <Link to="/explore" className="flex-1"><CustomButton title="Eksplorasi Proyek" styles="w-full bg-purple-500 hover:bg-purple-600" /></Link>
            <Link to="/history" className="flex-1"><CustomButton title="Riwayat Donasi Saya" styles="w-full bg-gray-600 hover:bg-gray-700" /></Link>
        </div>
      </div>
    </div>
  );
...
const AdminDashboardContent = ({ user, navigate }) => {
  ...
  // ... logika fetch data dashboard admin ...
  return (
    <div className="mt-8 space-y-8 animate-fadeInUp">
        ...
        <div className="bg-[#1c1c24] p-6 rounded-xl shadow-lg mt-6">
            <h3 className="font-epilogue font-semibold text-xl text-white mb-4">Aksi Cepat Panel Admin</h3>
